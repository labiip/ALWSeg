...
from data_utils.data_loader import DataGenerator, CropResize, To_Tensor0, To_Tensor1, DataGeneratorval, Trunc_and_Normalize, RandomGenerator
...
class DiceLoss_s(nn.Module):
    def __init__(self, n_classes, ignore_index):
        super(DiceLoss_s, self).__init__()
    ...
    def _dice_mask_loss(self, score, target, mask, ignore_mask):
        target = target.float()
        mask = mask.float()
        smooth = 1e-10
        ...
        # print('ignore_mask.shape:',mask.shape)
        # print(ignore_mask.squeeze(1).shape)
        # intersect = torch.sum(score * target * mask * ignore_mask)
        # y_sum = torch.sum(target * target * mask * ignore_mask)
        # z_sum = torch.sum(score * score * mask * ignore_mask)
    ...
    def forward(self, inputs, target, mask=None, weight=None, softmax=False):
        ...
        class_wise_dice = []
        loss = 0.0
        ignore_mask = torch.ones_like(tar)
        ignore_mask[tar == 4] = 0

        if mask is not None:
            mask = self._one_hot_mask_encoder(mask)
            for i in range(0, self.n_classes):
                dice = self._dice_mask_loss(inputs[:, i], target[:, i], mask[:, i], ignore_mask)
        ...
        # return torch.log2((torch.exp(loss / self.n_classes) + torch.exp(-loss / self.n_classes)) / 2.0)

dice1_loss = DiceLoss_s(n_classes=4, ignore_index=4)

def mix_loss(output, img_l, patch_l, mask, l_weight=1.0, u_weight=0.5, unlab=False):
    CE = nn.CrossEntropyLoss(reduction='none', ignore_index=4)
    ...
    patch_mask = 1 - mask
    loss_dice = dice1_loss(output_soft, img_l.unsqueeze(1), mask.unsqueeze(1)) * image_weight
    loss_dice += dice1_loss(output_soft, patch_l.unsqueeze(1), patch_mask.unsqueeze(1)) * patch_weight
...
def dwt_init(x):
    ...
    # return torch.cat([x_LL, x_HL, x_LH, x_HH], 1)  # original
...
class SemanticSeg(object):
    ...
    def trainer(self,
        ...
        for epoch in range(self.start_epoch, self.n_epoch):
            ...
            record_sample_list(
                record_csv=os.path.join(log_dir,'unlabeled_list.csv'),
                key=str(epoch),
                sample_list=self.unlabeled_data_pool)
    ...
    def _get_samples_per_epoch(self, N_sample, sample_times, sample_mode):
        ...
        sample_strategy[0] = init_sample + 12  ## attention here
    ...
    def _train_on_epoch(self,
        ...
        for step, sample in enumerate(train_loader):
            ...
            labeled_sub_bs, unlabeled_sub_bs = int(data.shape[0] / 2), int((data.shape[0]) / 2)
            ...
            p_label_hot = _one_hot_encoder(4,p_label)
            ...
            # add pre_train of BCP
            img_a, img_b = data[:labeled_sub_bs], data[labeled_sub_bs:2*labeled_sub_bs]
            lab_a, lab_b = target_s[:labeled_sub_bs], target_s[labeled_sub_bs:2*labeled_sub_bs]
            img_mask, loss_mask = generate_mask(img_a)
            mix_input = img_a * img_mask + img_b * (1 - img_mask)

            noise = torch.clamp(torch.randn_like(data) * 0.1, -0.2, 0.2)
            noise_u = torch.clamp(torch.randn_like(data_u) * 0.1, -0.2, 0.2)
            noise_input = data + noise
            noise_input_u = data_u + noise_u
            ...
            with autocast(self.use_fp16):
                ...
                output_noise, fea_noise = net(noise_input)
                output_noise_soft = torch.softmax(output_noise, dim=1)

                out_mixl,_ = net(mix_input)
                ...
                pseudo_supervision1 = criteriond(output_soft, output2_soft)
                pseudo_supervision2 = criteriond(output2_soft, output_soft) # .unsqueeze(1)
                pseudo_supervision3 = criteriond(output_soft, output_noise_soft)
                pseudo_supervision4 = criteriond(output_noise_soft, output_soft)
                pseudo_supervision5 = criteriond(output2_soft, output_noise_soft)
                pseudo_supervision6 = criteriond(output_noise_soft, output2_soft)
                pseudo_supervision_noise = pseudo_supervision3 + pseudo_supervision4 + pseudo_supervision5 + pseudo_supervision6

                # pseudo_supervision1_u = criteriond(output_u_soft, output2_u_soft)
                # pseudo_supervision2_u = criteriond(output2_u_soft, output_u_soft) # .unsqueeze(1)
                # pseudo_supervision_u = pseudo_supervision1_u + pseudo_supervision2_u

                # contrastive loss
                # print('fea.shape:', fea.shape)
                p1 = similar_criterion(fea[:,0,:,:], fea_noise[:,0,:,:]) / 0.1
                n1 = similar_criterion(fea[:,1,:,:], fea_noise[:,2,:,:]) / 0.1
                n2 = similar_criterion(fea[:,2,:,:], fea_noise[:,1,:,:]) / 0.1
                m = 0.1
                m1 = 0.01
                cos_m = math.cos(m)
                sin_m = math.sin(m)
                ...
                s2t = cos_m * p1 - sin_m * torch.sqrt(
                    (1.0 - torch.pow(p1, 2)).clamp(0.0001, 1.0))
                s2t1 = cos_m1 * n1 + sin_m1 * torch.sqrt(
                    (1.0 - torch.pow(n1, 2)).clamp(0.0001, 1.0))
                s2t2 = cos_m1 * n2 + sin_m1 * torch.sqrt(
                    (1.0 - torch.pow(n2, 2)).clamp(0.0001, 1.0))
                ##
                contrast_loss_s2t = -torch.log(torch.exp(s2t) / (torch.exp(s2t) +
                                                                 torch.sum(
                                                                     torch.exp(s2t1) + torch.exp(
                                                                         s2t2))))

                # contrast_loss_s2t = -torch.log(torch.exp(s2t) / (torch.exp(s2t) +
                #                                                  torch.sum(
                #                                                      torch.exp(n1) + torch.exp(
                #                                                          n2))))

                # contrast_loss_inter = contrast_loss_s2t
                # contrast_loss = torch.mean(contrast_loss_inter)
                # chaoxiangsu pseudo
                loss_p1 = criteriondd(output_soft, p_label)
                loss_p2 = criteriondd(output2_soft, p_label)
                loss_p_noise = criteriondd(output_noise_soft, p_label)
                loss_p = loss_p1 + loss_p2 + loss_p_noise

                # mix_loss
                loss_dice_cp, loss_ce_cp = mix_loss(out_mixl, lab_a, lab_b, loss_mask, u_weight=1.0, unlab=True)
                loss_cp = 0.5 * (loss_dice_cp + loss_ce_cp)

                # with ema_model online
                loss_ema_l = criteriond(output_soft, ema_output_soft) + criteriond(output2_soft, ema_output_soft) #+ criteriond(output_noise_soft, ema_output_soft)
                # loss_ema_u = criteriond(output_u_soft, ema_output_u_soft) + criteriond(output2_u_soft, ema_output_u_soft)
                loss_ema = loss_ema_l #+ loss_ema_l

                ### synthesis pseudo labels
                ...
                # loss = 0.6 * criterion(output, target_s.long()) + 0.4 * criterion(output2, target_s.long()) + 0.4 * criterion(output_noise, target_s.long()) \
                #        + cp1 * (pseudo_supervision1 + pseudo_supervision2) + 0.3 * loss_p  + 0.5 * cp1*loss_ema  + 2.0 * loss_cp# + 0.1 * contrast_loss

                loss = 0.6 * criterion(output, target_s.long()) + 0.4 * criterion(output2, target_s.long()) + 0.4 * criterion(output_noise, target_s.long()) \
                       + cp1 * (pseudo_supervision1 + pseudo_supervision2) + 0.3 * loss_p #+ 0.05 * contrast_loss
            ...
            if epoch >= (self.warmup_epoch - self.sample_inteval):
                ...
                predictor_target = torch.from_numpy(compute_dice(output.detach(),target,ignore_index=-1,reduction=None)).cuda()  #NC
                ...
                predictor_target = torch.from_numpy(np.ones((data.size(0),self.num_classes),dtype=np.float32)*-1.0) #NC
    ...
    def _val_on_epoch(self,
        ...
        with torch.no_grad():
            for step, sample in enumerate(val_loader):
                ...
                # measure dice and record loss
                dice = compute_dice(output.detach(), target)
                # dice2 = compute_dice(output2.detach(), target)
                val_loss.update(loss.item(), data.size(0))
                val_dice.update(dice, data.size(0))
                # val_dice2.update(dice2, data.size(0))
                val_predictor_loss.update(predictor_loss.item(), data.size(0))

                ## save model2
                if epoch > 60 & epoch % 10 == 0:
                    save_mode_path = os.path.join("./model2/", 'model2_iter_{}_dice_{}.pth'.format(epoch, dice))
