...
def acc_predictor(seg_net,segnet1,predictor,unlabeled_data_pool,sample_loader,sample_nums,sample_weight=None,al_mode='single',score_type='mean'):
    ...
    with torch.no_grad():
        for step, sample in enumerate(sample_loader):
            ...
            with autocast(True):
                output,_ = seg_net(data)#NCHW
                output1 = segnet1(data)
                ...
                HD = np.zeros_like(predictor_output.cpu())

                # HD95
                for i in range(data.shape[0]):
                    first_metric = calculate_metric_percase(output[i].cpu().numpy() == 1,output1[i].cpu().numpy() == 1)
                    second_metric = calculate_metric_percase(output[i].cpu().numpy() == 2, output1[i].cpu().numpy() == 2)
                    third_metric = calculate_metric_percase(output[i].cpu().numpy() == 3, output1[i].cpu().numpy() == 3)
                    avg_hd95 = 1 / ((first_metric + second_metric + third_metric) /3.0)
                    HD = avg_hd95

                # scores = list(compute_score(pred_out=predictor_output,weights=sample_weight,score_type=score_type)) #N
                scores = list(np.add(compute_score(pred_out=predictor_output,weights=sample_weight,score_type=score_type), 0.0005*HD)) #N
